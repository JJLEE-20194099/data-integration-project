...
features_test_loader = IOReader('pkl', settings.FEATURES_TEST).read()
features_train_loader = IOReader('pkl', settings.FEATURES_TRAIN).read()
features_train, labels_train = FeatureExtraction(
    data=features_train_loader).read_feature()
features_test, labels_test = FeatureExtraction(
    data=features_test_loader).read_feature()
...
def logictic():
    ...
    param_grid = {'penalty': ['l1', 'l2'], 'max_iter': [
        20, 50, 70, 100, 200], 'C': [0.1, 10, 20, 30]}

    clf = GridSearchCV(lr, param_grid, refit=True, cv=5)
...
# logictic:  (0.68112745098039218, {'penalty': 'l2', 'C': 30, 'max_iter': 20}, 0.69019607843137254)
...
#     param_grid = {'alpha': [1, 0.1, 0.01], 'hidden_layer_sizes': [(20,), (100,), (100, 20)]}
