...
class FeatureExtraction(object):
    ...
    def __build_dataset(self):
        ...
        self.features = [self.get_dense(d['content']) for d in self.data]
        self.labels = [self.get_dense(d['category']) for d in self.data]
        i = 0
    ...
    def get_data_and_label_tfidf(self):
        print 'Building dataset'
        self.features = [' '.join(NLP(d['content']).get_words_feature()) for d in self.data]
